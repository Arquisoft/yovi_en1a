...
UI1 -> API : POST /validate-move (YEN)
activate API
API -> Rust : POST /validate-move (YEN)
activate Rust
Rust --> API : 200 OK (Move Valid)
deactivate Rust
API --> UI1 : 200 OK (Move Valid)
deactivate API
...
UI -> API : POST /validate (YEN Notation)
activate API
API -> Rust : POST /validate (YEN Notation)
activate Rust
Rust --> API : Result: "WIN_PLAYER_1" (YEN JSON)
deactivate Rust
API --> UI : Result: "WIN_PLAYER_1" (YEN JSON)
deactivate API
